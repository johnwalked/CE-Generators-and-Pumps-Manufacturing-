...
import { Mic, PhoneOff, Cog, Wifi } from 'lucide-react';
    ...
    <div className="w-full h-full flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-4xl px-6 flex flex-col items-center gap-12">
        ...
        <div className="text-center space-y-4">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-mono uppercase tracking-widest transition-all ${isConnected ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-slate-800/50 border-slate-700 text-slate-500'}`}>
            <Wifi className={`w-3 h-3 ${isConnected ? 'animate-pulse' : ''}`} />
            {isConnected ? "Live Connection Active" : "Standby Mode"}
          </div>
          
          <h1 className={`text-4xl md:text-6xl font-black tracking-tight transition-colors duration-500 ${isConnected ? 'text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]' : 'text-slate-600'}`}>
            {isConnected ? "Listening..." : "Engine Expert"}
          </h1>
          
          <p className="text-lg md:text-xl text-slate-400 max-w-lg mx-auto font-light leading-relaxed">
            {isConnected 
              ? "Speak naturally. I can analyze specs, recommend brands, and solve technical issues." 
              : "Connect for real-time technical assistance in 5 languages."}
          </p>
        </div>
        ...
        <div className="relative w-64 h-64 md:w-96 md:h-96 flex items-center justify-center">
          ...
          <div className="relative z-10 transition-transform duration-700 hover:scale-105">
            <Cog 
              className={`w-64 h-64 md:w-80 md:h-80 drop-shadow-2xl transition-all duration-1000 ${isConnected ? 'animate-spin text-blue-100 opacity-90' : 'animate-spin-slow text-slate-700 opacity-50'}`} 
              strokeWidth={0.5}
            />
            
            {/* Inner Core */}
            <div className="absolute inset-0 flex items-center justify-center">
               <div className={`w-32 h-32 rounded-full flex items-center justify-center backdrop-blur-xl border border-white/10 shadow-inner transition-all duration-500 ${isConnected ? 'bg-blue-600/20' : 'bg-slate-900/50'}`}>
                  {isConnected ? (
        ...
        <div className="flex flex-col items-center gap-4 z-20">
          {error && (
            <div className="animate-in fade-in slide-in-from-bottom-2 text-red-400 bg-red-950/30 px-4 py-2 rounded-lg border border-red-500/30 text-sm mb-2">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className={`group relative px-10 py-5 rounded-2xl font-bold text-lg uppercase tracking-widest transition-all duration-300 transform hover:scale-105 active:scale-95 ${
              isConnected 
          ...
          {!isConnected && (
             <p className="text-slate-500 text-sm">Tap to initialize neural voice connection</p>
          )}
        </div>
      </div>
    </div>
